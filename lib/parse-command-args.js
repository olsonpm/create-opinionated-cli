...
function parseCommandArgs(aCommand) {
  ...
  const commandHasRequiredArgs = fp.some(
    anArg => fp.includes('require', anArg.flags)
    , aCommand.args
  );
  if ((commandHasRequiredArgs && passedArgs.length === 0) || helpArg.isCliAliasOrName(passedArgs[0])) {
    showHelp(aCommand, (passedArgs.length === 0) ? 2 : 1);
    return;
  }
...
function showHelp(aCommand, std) {
  ...
  if (optionalArgs.length) {
    if (argDescriptions) argDescriptions += '\n\n';
